  ...
  <nav class="nav" role="navigation">
    ...
    <div class="max-centered">
        ...
        <main role="main">
            ...
            <mobile v-if="$q.screen.lt.sm" :users="users" />
            <tablet v-else-if="$q.screen.lt.md" :users="users" />
            <desktop v-else :users="users" />
            <!-- End: Implementation -->
        </main>
        <aside class="banner banner-bottom" role="doc-tip" aria-label="Submit a name">
            <img
                ...
                sizes="(min-width: 750px) 1440px, 100vw"
                class="banner__background" src="img/bg-people.png"
                alt=""
                role="none">
            <div class="banner__left">
...
export default {
  ...
  created () {
    this.users.forEach(element => {
      const totalVotes = (element.votes.positive + element.votes.negative)
      element.votes.positive_percent = ((element.votes.positive * 100) / totalVotes).toFixed(1)
      element.votes.negative_percent = ((element.votes.negative * 100) / totalVotes).toFixed(1)
      element.thumb_icon = element.votes.positive > element.votes.negative ? 'thumb_up' : 'thumb_down'
      element.thumb_color = element.votes.positive > element.votes.negative ? 'rgba(60, 187, 180, 0.8)' : '#FBBD4A'
      element.time_ago = moment(element.lastUpdated).fromNow()
    })
  }
}
</script>
